...
// Ensure form actions inside included admin main-content point back to this user dashboard
$dashboard_action = 'dashboard_user.php';

// Copy of admin dashboard layout/content, adapted for user role to match Kaiadmin style

// Data for 7-day cash flow chart
$chart_labels = [];
...
$pie_month = isset($_GET['pie_month']) ? intval($_GET['pie_month']) : intval(date('m'));
$pie_year = isset($_GET['pie_year']) ? intval($_GET['pie_year']) : intval(date('Y'));
...
// Bar chart per user
$bar_chart_month = isset($_GET['bar_chart_month']) ? intval($_GET['bar_chart_month']) : intval(date('m'));
$bar_chart_year = isset($_GET['bar_chart_year']) ? intval($_GET['bar_chart_year']) : intval(date('Y'));
...
// Radar data
$total_pengumuman = 0;
$stmt_pengumuman = $conn->prepare("SELECT COUNT(*) as total FROM announcements");
...
$total_agenda = 0;
$stmt_agenda = $conn->prepare("SELECT COUNT(*) as total FROM jadwal_kegiatan");
$stmt_agenda->execute();
...
<head>
    ...
    <title>Kaiadmin - User Dashboard</title>
    <meta
...
<body>
    <div class="wrapper">
        ...
        <div class="main-panel">
            ...
            <!-- main-content -->
            <?php include __DIR__ . '/../admin/layout_admin/main-content.php'; ?>
    ...
    <script>
        // Logout confirmation (if navbar includes logout-link)
    ...
    <script>
        // Live clock (WIB / Asia/Jakarta) — updates every second
        (function() {
            function updateClock() {
                ...
                const m = {};
                parts.forEach(p => {
                    if (p.type) m[p.type] = p.value;
                });
                const weekday = m.weekday || '';
                const day = m.day || '00';
                const month = m.month || '';
                const year = m.year || '';
                const hour = m.hour || '00';
                const minute = m.minute || '00';
                const second = m.second || '00';
                const formatted = `${weekday} ${day} ${month} ${year} ${hour}:${minute}:${second} WIB`;
