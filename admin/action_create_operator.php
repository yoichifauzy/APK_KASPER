...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: create_operator.php');
    exit;
}
...
if ($nama === '' || $username === '' || $password === '') {
    header('Location: create_operator.php?error=missing');
    exit;
}
...
$stmt->store_result();
if ($stmt->num_rows > 0) {
    header('Location: create_operator.php?error=exists');
    exit;
}
...
if (!empty($_FILES['profile_picture']['name'])) {
    ...
    if ($up['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array(strtolower($ext), $allowed)) {
            header('Location: create_operator.php?error=badfile');
            exit;
        }
...
if ($ins->execute()) {
    header('Location: manage_operator.php?msg=created');
    exit;
} else {
    header('Location: create_operator.php?error=insert');
    exit;
}
