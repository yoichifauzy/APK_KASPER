...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: manage_operator.php');
    exit;
}
...
if ($id <= 0 || $nama === '' || $username === '') {
    header('Location: edit_operator.php?id=' . $id . '&error=missing');
    exit;
}
...
$stmt->store_result();
if ($stmt->num_rows > 0) {
    header('Location: edit_operator.php?id=' . $id . '&error=exists');
    exit;
}
...
if (!empty($_FILES['profile_picture']['name'])) {
    ...
    if ($up['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array(strtolower($ext), $allowed)) {
            header('Location: edit_operator.php?id=' . $id . '&error=badfile');
            exit;
        }
...
if ($upd->execute()) {
    header('Location: manage_operator.php?msg=updated');
    exit;
} else {
    header('Location: edit_operator.php?id=' . $id . '&error=update');
    exit;
}
